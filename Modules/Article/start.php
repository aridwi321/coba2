...
\Menu::makeOnce('admin_sidebar', function ($menu) {
    ...
    // Articles Dropdown
    $articles_menu = $menu->add('<i class="nav-icon fas fa-file-alt"></i> Trash', [
    ...
    $articles_menu->link->attr([
     ...
     // Submenu: Inputs
     $articles_menu->add('<i class="nav-icon fas fa-file-alt"></i> Inputs', [
        'route' => 'backend.inputs.index',
        'class' => 'nav-item',
    ])
    ->data([
        'order'         => 82,
        'activematches' => 'admin/inputs*',
        'permission'    => ['edit_inputs'],
    ])
    ->link->attr([
        'class' => 'nav-link',
    ]);
    ...
    // Submenu: Crafts
    $articles_menu->add('<i class="nav-icon fas fa-file-alt"></i> Crafts', [
        'route' => 'backend.crafts.index',
        'class' => 'nav-item',
    ])
    ->data([
        'order'         => 84,
        'activematches' => 'admin/crafts*',
        'permission'    => ['edit_crafts'],
    ])
    ->link->attr([
        'class' => 'nav-link',
    ]);
